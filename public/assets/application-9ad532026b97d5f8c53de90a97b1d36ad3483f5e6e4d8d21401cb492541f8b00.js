...
var PopulationCapacityChart = (function () {
  ...
  _createClass(PopulationCapacityChart, [{
    ...
    value: function render(targetElement) {
      ...
      var markerLabels = objectValues(this.labels);
      ...
      var markerAxis = d3.axisTop(xScale).tickSize(height).tickFormat(function (d) {
        var i = markerValues.indexOf(d);return markerLabels[i];
    ...
    key: 'appendPatterns',
    value: function appendPatterns(svg) {
      var iconSize = 48;
      var iconPadding = 12;
      var patternHeight = 85;

      var patternDefs = svg.append('defs');

      patternDefs.append('pattern').attr('id', 'bed').attr('patternUnits', 'userSpaceOnUse').attr('width', iconSize + iconPadding).attr('height', patternHeight).append('image').attr('xlink:href', '/assets/bed.svg').attr('width', iconSize).attr('height', iconSize);

      patternDefs.append('pattern').attr('id', 'boat').attr('patternUnits', 'userSpaceOnUse').attr('width', iconSize + iconPadding).attr('height', patternHeight).append('image').attr('xlink:href', '/assets/boat.svg').attr('width', iconSize).attr('height', iconSize);
    }
  }, {
    key: 'drawBar',
    value: function drawBar(chart, xScale, height) {
      var boatsInUse = this.args.active_bookings > this.args.markers.red_zone_start;

      var bedsBarUpperLimit = Math.min(this.args.markers.red_zone_start, this.args.active_bookings);

      chart.append('rect').attr('class', 'active-bookings active-booking-beds').attr('width', xScale(bedsBarUpperLimit)).attr('y', height / 2).attr('fill', 'url(#bed)').attr('height', height / 2);

      if (boatsInUse) {
        chart.append('rect').attr('class', 'active-bookings active-booking-boats').attr('width', xScale(this.args.active_bookings - bedsBarUpperLimit)).attr('y', height / 2).attr('x', xScale(bedsBarUpperLimit)).attr('fill', 'url(#boat)').attr('height', height / 2);
      }
    }
  }, {
    key: 'redraw',
...
var Filter = (function () {
  function Filter(data, filters, filterElement, tableCallback) {
  ...
  _createClass(Filter, [{
    key: 'processDates',
    value: function processDates(data) {
    ...
    value: function render() {
      ...
      this.dimensions.status = bookings.dimension(function (d) {
        return d.status;
      });
      this.dimensions.location = bookings.dimension(function (d) {
        return d.facility_name;
      });
      this.dimensions.gender = bookings.dimension(function (d) {
        return d.gender;
      });
      this.dimensions.race = bookings.dimension(function (d) {
        return d.race;
      });
      this.dimensions.lengthOfStay = bookings.dimension(this.lengthOfStay);
      this.dimensions.category = bookings.dimension(function (d) {
        return d.category;
      });
      this.dimensions.description = bookings.dimension(function (d) {
        return d.description;
      });
      this.dimensions.bookingDateTime = bookings.dimension(function (d) {
        return d.booking_date_time;
      });
      this.dimensions.releaseDateTime = bookings.dimension(function (d) {
        return d.release_date_time;
      });

      this.filterLabels().forEach(function (filter) {
        return _this.addBreakdownChart(filter);
      });
    ...
    key: 'filterLabels',
    value: function filterLabels() {
      return Object.keys(this.filters);
    }
  }, {
    key: 'addBreakdownChart',
    value: function addBreakdownChart(dimensionName) {
      var _this2 = this;

      var breakdownElement = this.filterElement.append('div').attr('class', 'breakdown-' + dimensionName);

      breakdownElement.append('h3').text('Filter by ' + dimensionName);

      var breakdownBars = breakdownElement.append('div').attr('class', 'breakdown-bars ' + dimensionName);

      var breakdownTable = breakdownElement.append('table').attr('class', 'table table-striped table-responsive breakdown-table ' + dimensionName);

      this.breakdown(dimensionName).forEach(function (segment) {
        var className = _this2.classify(segment.key);

        breakdownBars.append('div').attr('class', 'breakdown-bar ' + className);

        breakdownTable.append('tr').attr('class', 'breakdown-row ' + className).attr('role', 'link').on('click', function () {
          _this2.filters[dimensionName][segment.key] = !_this2.filters[dimensionName][segment.key];
          _this2.update();
        });
      });

      breakdownElement.append('button').text('Clear ' + dimensionName + ' filters').on('click', function () {
        Object.keys(_this2.filters[dimensionName]).forEach(function (filterName) {
          _this2.filters[dimensionName][filterName] = false;
        });
    ...
    key: 'classify',
    value: function classify(string) {
    ...
    key: 'breakdown',
    value: function breakdown(dimensionName) {
      return this.dimensions[dimensionName].group().reduceCount().top(Infinity);
    }
  }, {
    key: 'onUpdate',
    ...
    value: function update() {
      ...
      Object.keys(this.filters).forEach(function (dimensionName) {
        if (_this3.filtersAllInactive(dimensionName)) _this3.dimensions[dimensionName].filterAll();else _this3.dimensions[dimensionName].filterFunction(function (d) {
          return _this3.filters[dimensionName][d];
        });
      });
      ...
      var selectedBookings = this.dimensions.lengthOfStay.top(Infinity);
      this.onUpdateCallbacks.forEach(function (callback) {
        return callback(selectedBookings);
      });

      // collapse all the data down into the currently active ones
      var currentDate = this.activeDateTime;
      this.dimensions.bookingDateTime.filterFunction(function (d) {
        return d < currentDate;
      });
      this.dimensions.releaseDateTime.filterFunction(function (d) {
        return d == null || d > currentDate;
      });

      // call update callbacks with current bookings
      var selectedBookingsAtSinglePointInTime = this.dimensions.lengthOfStay.top(Infinity);
      this.tableCallback(selectedBookingsAtSinglePointInTime);

      this.filterLabels().forEach(function (dimensionName) {
        var totalCount = _this3.dimensions[dimensionName].groupAll().reduceCount().value();

        _this3.breakdown(dimensionName).forEach(function (segment) {
          var breakdownBars = _this3.filterElement.select('.breakdown-bars.' + dimensionName);

          var breakdownTable = _this3.filterElement.select('.breakdown-table.' + dimensionName);

          var className = _this3.classify(segment.key);

          breakdownBars.selectAll('.breakdown-bar.' + className).style('flex', '0 1 ' + _this3.percentage(segment.value, totalCount)).classed('active', _this3.filtersAllInactive(dimensionName) || _this3.filters[dimensionName][segment.key]);

          var row = breakdownTable.select('.breakdown-row.' + className);

          row.classed('active', _this3.filtersAllInactive(dimensionName) || _this3.filters[dimensionName][segment.key]);

          row.selectAll('th, td').remove();
          row.append('th').text(segment.key);
          row.append('td').attr('class', 'breakdown-value').text(segment.value);
          row.append('td').attr('class', 'breakdown-percentage').text(_this3.percentage(segment.value, totalCount));
        });
      });
    }
  }, {
    key: 'filtersAllInactive',
    value: function filtersAllInactive(dimensionName) {
      var _this4 = this;

      return Object.keys(this.filters[dimensionName]).map(function (d) {
        return _this4.filters[dimensionName][d];
      }).indexOf(true) == -1;
    }
  }, {
    key: 'lengthOfStay',
    value: function lengthOfStay(booking) {
      if (booking.release_date_time) return new Date(booking.release_date_time) - new Date(booking.booking_date_time);else return new Date() - new Date(booking.booking_date_time);
    }
  }, {
    key: 'distanceOfTimeInWords',
    value: function distanceOfTimeInWords(duration) {
      ...
      if (distance_in_minutes < 1440) {
        return 'about ' + Math.floor(distance_in_minutes / 60) + ' hours';
      }
      if (distance_in_minutes < 2880) {
        return 'a day';
      }
      if (distance_in_minutes < 43200) {
        return Math.floor(distance_in_minutes / 1440) + ' days';
      }
      if (distance_in_minutes < 86400) {
        return 'about a month';
      }
      if (distance_in_minutes < 525960) {
        return Math.floor(distance_in_minutes / 43200) + ' months';
      }
    ...
    key: 'percentage',
    value: function percentage(partial, total) {
      var percentageScale = d3.scaleLinear().domain([0, total]).range([0, 100]);
...
var HistoricalChart = (function () {
  function HistoricalChart(data, chartElement) {
    _classCallCheck(this, HistoricalChart);
  ...
  _createClass(HistoricalChart, [{
    key: 'filtersUpdated',
    value: function filtersUpdated(data) {
      this.data = data;
      this.update();
    }
  }, {
    ...
    value: function render() {
      ...
      var svg = d3.select('.historical').append('svg').datum(data).call(xy_chart);
    }
  }, {
    key: 'd3_xy_chart',
    value: function d3_xy_chart() {
      ...
      function chart(selection) {
        selection.each(function (datasets) {
          ...
          var x_scale = d3.scaleTime().range([0, innerwidth]).domain([d3.min(datasets, function (d) {
            return d3.min(d.date);
          }), d3.max(datasets, function (d) {
            return d3.max(d.date);
          })]);
          var y_scale = d3.scaleLinear().range([innerheight, 0]).domain([0, d3.max(datasets, function (d) {
            return d3.max(d.count);
          })]);
          var x_axis = d3.axisBottom(x_scale);
          var y_axis = d3.axisLeft(y_scale);
          var draw_line = d3.line().x(function (d) {
            return x_scale(d[0]);
          }).y(function (d) {
            return y_scale(d[1]);
          });
          ...
          svg.append('g').attr('class', 'x axis').attr('transform', 'translate(0,' + innerheight + ')').call(x_axis).append('text').attr('dy', '-.71em').attr('x', innerwidth).style('text-anchor', 'end');

          svg.append('g').attr('class', 'y axis').call(y_axis).append('text').attr('transform', 'rotate(-90)').attr('y', 6).attr('dy', '0.71em').style('text-anchor', 'end');

          var data_lines = svg.selectAll('.d3_xy_chart_line').data(datasets.map(function (d) {
            return d3.zip(d.date, d.count);
          })).enter().append('g').attr('class', 'd3_xy_chart_line');

          data_lines.append('path').attr('class', 'line').attr('d', function (d) {
            return draw_line(d);
          }).attr('fill', 'white').attr('stroke', 'lightseagreen');
        });
      }
    ...
    key: 'getCounts',
    value: function getCounts(dates) {
      var bookings = crossfilter(this.data);
      var counts = [];
      dates.forEach(function (date) {
        var currentDate = date;
        var bookingDateTime = bookings.dimension(function (d) {
          return d.booking_date_time;
        });
        var releaseDateTime = bookings.dimension(function (d) {
          return d.release_date_time;
        });
        bookingDateTime.filterFunction(function (d) {
          return d < currentDate;
        });
        releaseDateTime.filterFunction(function (d) {
          return d == null || d > currentDate;
        });
...
var Table = (function () {
  ...
  _createClass(Table, [{
    key: 'render',
    value: function render() {
      this.table = this.tableElement.append('table').attr('class', 'table table-striped table-responsive');
    ...
    key: 'update',
    value: function update(bookings) {
      var _this = this;

      var update = this.body.selectAll('tr').data(bookings);

      update.exit().remove();

      var enter = update.enter().append('tr');

      update.merge(enter).html(function (d) {
        var lengthOfStay = _this.lengthOfStay(d);

        return ['<td>' + d.first_name + ' ' + d.last_name + '</td>', '<td>' + d.jms_person_id + '</td>', '<td>' + d.status + '</td>', '<td>' + d.facility_name + '</td>', '<td>' + _this.distanceOfTimeInWords(lengthOfStay) + '</td>'].join('');
      });
    }
  }, {
    key: 'lengthOfStay',
...
window.runApplication = function () {
  ...
  var table = new Table(d3.select('.filtered-people'));
  table.render();

  var historical = new HistoricalChart(d3.select('.historical'));

  var filter = new Filter(gon.crossfilter_data, gon.filters, d3.select('.filters'), function (d) {
    return table.update(d);
  });

  filter.onUpdate(function (data) {
    return historical.filtersUpdated(data);
  });
  filter.render();
};
